...
## Test Suite 3 – Officer Performance Report
**Test Case IDs:** TC_PERFORMANCE_REPORT_01, TC_PERFORMANCE_REPORT_02  

---

### Test Case 1
**Test Case ID:** TC_PERFORMANCE_REPORT_01  
**Test Priority:** High  
**Module Name:** GoodPoliz: Officer Performance Report  
**Test Title:** Generate performance report for current month (Happy Path)  
**Description:** Verify that when the user selects **today** as the report date (current month), the system generates an officer performance report that summarizes all data for the **current month**. The report should display non-empty statistics for messages and incidents that occurred in this month.
...
#### Pre-conditions
- User can log in to the **GoodPoliz** system.
- There are incident and/or messaging records created **in the current month**.
- The **Officer Performance Report** page is available in the menu.

#### Dependencies
Login page, Dashboard/Home page, Officer Performance Report page, Performance Summary API (`/api/v1/performance/summary`)

| Step | Test Steps | Test Data | Expected Result | Actual Result | Status (Pass/Fail) | Notes |
|---:|---|---|---|---|---|---|
| 1 | Log in and navigate to the **Officer Performance Report** page. | Email/Pass: **Earn / Earn** | System logs in successfully and displays the Officer Performance Report page. | System logs in successfully and displays the Officer Performance Report page. | Pass |  |
| 2 | In the **date/period selector**, select **Today** (a date in the current month). | Today’s date (e.g., `2025-11-10`) | The date/period selector accepts the input and shows the selected date. | The date/period selector accepts the input and shows the selected date. | Pass |  |
| 3 | (If available) Keep **Officer** filter as **All** or default. | Officer: *All* | Officer filter remains at default (All). | Officer filter remains at default (All). | Pass |  |
| 4 | Click **Generate Report** / **View Report** button. | – | System calls the performance summary API for the current month range and loads the performance summary. | System calls the performance summary API for the current month range and loads the performance summary. | Pass |  |
| 5 | Review the **performance summary** section (messaging & incidents). | – | The report shows non-zero or meaningful counts for current-month data (e.g., total messages, total incidents, rank distribution). No “No data” message is shown for the whole report. | The report shows non-zero or meaningful counts for current-month data. No “No data” message is shown for the whole report. | Pass |  |
| 6 | (Optional) Click **Export / Download PDF** for the performance report. | – | System generates a PDF with the same summary for the selected period (current month). | System generates a PDF with the same summary for the selected period (current month). | Pass |  |

#### Post-conditions
- A performance summary for the **current month** is displayed.
- The officer performance report correctly reflects existing data from the current month (non-empty statistics).

---

### Test Case 2
**Test Case ID:** TC_PERFORMANCE_REPORT_02  
**Test Priority:** Medium  
**Module Name:** GoodPoliz: Officer Performance Report  
**Test Title:** Generate performance report for last month with no data (Unhappy/Empty Data Path)  
**Description:** Verify that when the user selects a date in **last month**, and there is **no data** stored for that month, the system generates an empty performance report — no incidents or messages are shown, and an appropriate “no data” state is displayed.
...
#### Pre-conditions
- User can log in to the **GoodPoliz** system.
- There are **no** incident or messaging records for **last month** (data was only created starting from the current month).
- The **Officer Performance Report** page is available in the menu.
...
|---:|---|---|---|---|---|---|
| 1 | Log in and navigate to the **Officer Performance Report** page. | Email/Pass: **Earn / Earn** | System logs in successfully and displays the Officer Performance Report page. | System logs in successfully and displays the Officer Performance Report page. | Pass |  |
| 2 | In the **date/period selector**, select a date in **last month**. | Example: if current month is Nov 2025, select `2025-10-10`. | The date/period selector accepts the input and shows the selected date (last month). | The date/period selector accepts the input and shows the selected date (last month). | Pass |  |
| 3 | (If available) Keep **Officer** filter as **All** or default. | Officer: *All* | Officer filter remains at default (All). | Officer filter remains at default (All). | Pass |  |
| 4 | Click **Generate Report** / **View Report** button. | – | System calls the performance summary API for the last-month range and attempts to load data. | System calls the performance summary API for the last-month range and attempts to load data. | Pass |  |
| 5 | Observe the **performance summary** section. | – | No incidents or messages are shown. All counts are zero or an appropriate “No data available for this period” message is displayed. No error occurs. | No incidents or messages are shown. All counts are zero and a “No data available” message is displayed. No error occurs. | Pass |  |
| 6 | (Optional) Try exporting the report as PDF. | – | System generates an empty/zero-value PDF report for last month, consistent with no data. | System generates an empty/zero-value PDF report for last month, consistent with no data. | Pass |  |

#### Post-conditions
- No officer performance data is displayed for **last month**.
- The system correctly handles an empty-data period by showing an empty/“no data” report instead of crashing or showing wrong values.
